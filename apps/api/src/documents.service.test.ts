...
describe('DocumentService boundaries', () => {
  ...
  it('rejects likely healthcare content before storing it', () => {
    ...
    expect(() =>
      service.create(ownerA, {
        title: 'Consent',
        text: 'Patient medical consent for hospital care.',
        mimeType: 'text/plain',
      }),
    ...
    expect(() =>
      service.create(ownerA, {
        title: 'Authorization',
        text: 'The clinic may renew this chemotherapy treatment authorization.',
        mimeType: 'text/plain',
      }),
    ).toThrow(BadRequestException);
  ...
  it('fails closed when the document domain cannot be confidently allowlisted', () => {
    ...
    expect(() =>
      service.create(ownerA, {
        title: 'Unknown',
        text: 'A dense document with no supported domain markers.',
        mimeType: 'text/plain',
      }),
    ).toThrow(BadRequestException);
    expect(service.list(ownerA)).toEqual([]);
  ...
  it('fails closed for domain packs not enabled in the consumer MVP', () => {
    ...
    expect(() =>
      service.create(ownerA, {
        title: 'Lease',
        text: 'Tenant shall pay rent to the landlord.',
        mimeType: 'text/plain',
      }),
    ).toThrow(BadRequestException);
  });
  ...
  it('does not disclose a document to another local session', () => {
    const service = new DocumentService();
    const record = service.create(ownerA, {
      title: 'Terms',
  ...
  it('supports distinct evidence-linked dispute drafts', () => {
    ...
    const draft = service.createDraft(ownerA, record.id, {
      clauseIds: [clauseId],
